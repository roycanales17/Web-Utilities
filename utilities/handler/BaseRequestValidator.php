				...
				foreach ($rules as $rule) {
					...
					if ($rule === 'confirmed') {
						...
						if ($value !== $this->input($confirmationField)) {
							$this->addError($field, "$field does not match $confirmationField.");
						}
						continue;
					}
					...
					if (str_starts_with($rule, 'callback:')) {
						...
						if (is_callable($callback)) {
							...
							if ($result !== true) {
								$this->addError($field, $result ?: "$field failed custom validation.");
							}
						}
						continue;
					}

					// Apply standard and file/image rules
		...
		private function applyRule(string $field, string $rule, $value): void
		{
			...
			if (str_starts_with($rule, 'max:')) {
				...
				if (strlen((string)$value) > $max) {
					$this->addError($field, "$field must be less than $max characters.");
				}
			}
			...
			if (str_starts_with($rule, 'same:')) {
				...
				if ($value !== $this->input($other)) {
					$this->addError($field, "$field must match $other.");
				}
			}
			...
			if (str_starts_with($rule, 'not_in:')) {
				...
				if (in_array($value, $blocked)) {
					$this->addError($field, "$field contains a blocked value.");
				}
			}
			...
			foreach ($fileList as $file) {
				...
				if ($rule === 'file' && (!$file || $file['error'] !== UPLOAD_ERR_OK)) {
					$this->addError($field, "$field must be a valid uploaded file.");
				}

				// image rule
				if ($rule === 'image' && (!$file || !@getimagesize($file['tmp_name']))) {
					$this->addError($field, "$field must be a valid image.");
				}
				...
				if (str_starts_with($rule, 'max_size:')) {
					...
					if ($file && $file['size'] > ($maxKb * 1024)) {
						$this->addError($field, "$field size must be less than {$maxKb}KB.");
					}
				}
				...
				if (str_starts_with($rule, 'dimensions:')) {
					if ($file && $dim = @getimagesize($file['tmp_name'])) {
						...
						if (isset($rulesDim['min_width']) && $width < $rulesDim['min_width']) {
							$this->addError($field, "$field width must be at least {$rulesDim['min_width']}px.");
						}
						if (isset($rulesDim['min_height']) && $height < $rulesDim['min_height']) {
							$this->addError($field, "$field height must be at least {$rulesDim['min_height']}px.");
						}
						if (isset($rulesDim['max_width']) && $width > $rulesDim['max_width']) {
							$this->addError($field, "$field width must be less than {$rulesDim['max_width']}px.");
						}
						if (isset($rulesDim['max_height']) && $height > $rulesDim['max_height']) {
							$this->addError($field, "$field height must be less than {$rulesDim['max_height']}px.");
						}
					}
				}
			}
			...
			if (str_starts_with($rule, 'max_files:') && $isMultiple) {
				...
				if (count($fileList) > $maxFiles) {
					$this->addError($field, "You can upload a maximum of {$maxFiles} files for {$field}.");
				}
			}
		}
	}